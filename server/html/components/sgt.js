...
class SGT_template{
		...
		if (studentName==="" || studentCourse==="" || studentGrade===""){
			...
			var ajaxObj = {
				...
				success: function(result){
					console.log(result);
					SGT.createStudent(studentName, studentCourse, studentGrade, result.new_id);
					SGT.getServerData(result);
				},
				error: function(result){
					SGT.handleServerError(result);
				}
			}
			$.ajax(ajaxObj);
			this.clearInputs();
	...
	*/
	...
	handleServerData(serverObj){
		for (var studentIndex=0; studentIndex<serverObj.data.length; studentIndex++){
			this.createStudent(serverObj.data[studentIndex].name, 
								serverObj.data[studentIndex].course, 
								serverObj.data[studentIndex].grade, 
								serverObj.data[studentIndex].id);
		}
		this.displayAllStudents();
	}

	handleServerError(serverObj){
